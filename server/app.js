...
async function doSwapAction(data) {
  ...
  let token721Contract = new ethers.Contract(data.tokenaddress, ERC721_ABI, account);
  // Create random
  ...
  if (rndInt === 1) {
    ...
    let tokenamount_router = await router.getAmountsOut(amountIn, [
      process.env.WBNB_ADDR,
      data.tokenaddress,
    ]);
    ...
    console.log(
      ...
      bnbAmount,
      'Token - ',
      ethers.utils.formatEther(tokenamount_router[1].toString())
    );

    if (tokenamount_router[1] > tokenbalance_wallet) {
      console.log(
        'Info: current token balance in wallet - ',
        ethers.utils.formatEther(tokenbalance_wallet.toString())
      );
        ...
        try {
          const approve = await tokenContract
            .approve(
              ...
              { gasLimit: 100000, gasPrice: ethers.utils.parseUnits(`10`, 'gwei') }
            )
            .catch((err) => {
              console.log(err);
